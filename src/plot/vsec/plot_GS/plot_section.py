...
def mpl_sec_get_section(lon3, lat3, rbat2, zenv2, sec_i, sec_j, coord_type, \
    ...
    if isinstance(sec_i, list) and isinstance(sec_j, list):
       ...
       if sec_i == -1:
          ...
          secI        = np.arange(ni)
          secJ        = np.multiply(np.ones(ni, dtype=np.int32),sec_j)
          sec_name    = "j_" + str(sec_j)

       if sec_j == -1:

          secI        = np.multiply(np.ones(nj, dtype=np.int32),sec_i)
          secJ        = np.arange(nj)
...
def mpl_sec_bathy(rbat, rbat_fill, zenv, x2, tdep2, wdep2, tmsk2, mbat_fill, \
    ...
    if tdep2 is not None and wdep2 is not None and tmsk2 is not None and vlevel != "no":
       ...
       MB = plt.plot(mbat_x[:-2], mbat_z[:-2], color='black', label='Model Bathymetry')
    ...
    if zenv is not None:
       ...
       for nenv in range(num_env):
           ...
           plt.setp(ZE, 'linewidth', 6., zorder=20)    
...
def mpl_sec_settings(var2, x2, dep2, varlim, xlim, ylim):
    ...
    if (isinstance(var2, np.ndarray) and var2.size) and varlim != []:
...
def mpl_sec(exp_name, fig_type, var_strng, unit_strng, date, time, timestep, PlotType, sec_i, sec_j,
    ...
    if PlotType == "pcolor":
       ...
       if var2_aux != [] and var2_aux is not None:
    ...
    if (isinstance(rbat2, np.ndarray) and rbat2.size) or mbat_ln == "true":
...
def mpl_sec_loop(exp_name, fig_type, var_strng, unit_strng, date, timeres, timestep, PlotType, \
    ...
    if isinstance(var4, list) and var4 == []:
       print("")
       print("=========================================")
    ...
    for s in np.arange(len(sec_i)):
        ...
        if plot_geom:
           ...
           if isinstance(timestep, (str, bytes)):
              ...
              for t in timestep:
                  print("time step: ", t)
                  if (isinstance(var4, np.ndarray) and var4.size):
                     var3              = var4[t,:,:,:]
                     var3[tmsk3 == 0.] = np.nan
                  else:
                     var3 = []
